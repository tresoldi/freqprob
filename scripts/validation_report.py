...
class ValidationReportGenerator:
    ...
    def run_performance_benchmarks(self) -> Dict[str, Any]:
        ...
        performance_results['creation_scaling'] = {
            method: [m.to_dict() for m in metrics]
            for method, metrics in scaling_results.items()
        }
        ...
        query_results = self.benchmarker.benchmark_query_scaling(
            test_methods, query_counts
        )
    ...
    def generate_summary_statistics(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'statistical_correctness' in results:
            correctness_passed = sum(
                1 for result in results['statistical_correctness'].values()
                if result['passed']
            )
            ...
            summary['validation_overview']['statistical_correctness'] = {
                ...
                'success_rate': correctness_passed / correctness_total if correctness_total > 0 else 0
            }
...
"""
            for method, data in results['numerical_stability'].items():
                success_rate = data['passed'] / data['total_tests'] if data['total_tests'] > 0 else 0
...
"""
    ...
    def run_full_validation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.report_data['test_results']['numerical_stability'] = self.run_numerical_stability_tests()
            self.report_data['test_results']['statistical_correctness'] = self.run_statistical_correctness_tests()
            self.report_data['test_results']['regression_tests'] = self.run_regression_tests()
            self.report_data['test_results']['property_based_tests'] = self.run_property_based_tests()
            
            self.report_data['performance_results']['performance_benchmarks'] = self.run_performance_benchmarks()
            
            # Generate summary
            self.report_data['summary'] = self.generate_summary_statistics(self.report_data['test_results'])
            ...
            combined_results = {**self.report_data['test_results'], **self.report_data['performance_results']}
    ...
    if not args.quiet:
        ...
        if 'test_results' in results:
            ...
            if 'statistical_correctness' in results['test_results']:
                ...
                print(f"Statistical Correctness: {passed_correctness}/{total_correctness} tests passed")
        
        print(f"\nDetailed reports saved to: {args.output_dir}")
        print(f"- validation_report.html (human-readable)")
        print(f"- validation_report.json (detailed data)")
        print(f"- profiler_results.json (performance data)")
        print(f"- benchmark_results.json (benchmark data)")
