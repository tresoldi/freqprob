...
from freqprob import (
    StreamingFrequencyDistribution, StreamingMLE, StreamingLaplace, StreamingDataProcessor,
    CompressedFrequencyDistribution, SparseFrequencyDistribution, QuantizedProbabilityTable,
    create_compressed_distribution, create_sparse_distribution,
    MemoryProfiler, DistributionMemoryAnalyzer, MemoryMonitor
)
from freqprob.streaming import IncrementalScoringMethod
from freqprob.profiling import profile_memory_usage, get_object_memory_usage
...
class TestMemoryEfficientRepresentations:
    ...
    def test_factory_functions(self):
        ...
        quantized = create_compressed_distribution(
            original_data, quantization_levels=16
        )
...
class TestMemoryManagementIntegration:
    ...
    def test_end_to_end_memory_efficiency(self):
        ...
        has_savings = any(
            savings[method]['percentage_savings'] > 0 
            for method in savings
        )
        assert has_savings
