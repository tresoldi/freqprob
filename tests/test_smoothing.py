...
def test_interpolated_smoothing_basic():
    ...
    interp = InterpolatedSmoothing(
        high_order, low_order, lambda_weight=0.7, logprob=False
    )
...
def test_interpolated_smoothing_weights():
    ...
    interp_high = InterpolatedSmoothing(
        high_order, low_order, lambda_weight=0.9, logprob=False
    )
    
    # Test with high weight on low-order model
    interp_low = InterpolatedSmoothing(
        high_order, low_order, lambda_weight=0.1, logprob=False
    )
