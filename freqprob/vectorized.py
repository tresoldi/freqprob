...
from .base import ScoringMethod, Element
...
class VectorizedScorer:
    ...
    def score_batch(self, elements: Union[List[Element], np.ndarray]) -> np.ndarray:
        ...
        indices = np.array([
            self._element_cache.get(elem, -1) for elem in elements
        ], dtype=np.int32)
...
class BatchScorer:
    ...
    def benchmark_methods(self, elements: List[Element], num_iterations: int = 100) -> Dict[str, float]:
        """
