...
from .base import ScoringMethod, ScoringMethodConfig, FrequencyDistribution, Element
...
class LazyProbabilityComputer(ABC):
    ...
    @abstractmethod
    def compute_probability(self, element: Element, freqdist: FrequencyDistribution, config: ScoringMethodConfig) -> float:
        """
    ...
    @abstractmethod
    def compute_unobserved_probability(self, freqdist: FrequencyDistribution, config: ScoringMethodConfig) -> float:
        """
...
class LazyScoringMethod(ScoringMethod):
    ...
    def __init__(self, lazy_computer: LazyProbabilityComputer, config: ScoringMethodConfig, name: str):
        """
    ...
    def _get_unobserved_probability(self) -> float:
        ...
        if not self._unobs_computed:
            unobs_prob = self.lazy_computer.compute_unobserved_probability(self._freqdist, self.config)
...
def create_lazy_mle(freqdist: FrequencyDistribution, unobs_prob: Optional[float] = None, logprob: bool = True) -> LazyScoringMethod:
    """
...
def create_lazy_laplace(freqdist: FrequencyDistribution, bins: Optional[int] = None, logprob: bool = True) -> LazyScoringMethod:
    """
