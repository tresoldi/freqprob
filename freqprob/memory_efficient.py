...
import bisect
import sys
from .base import Element, Count, FrequencyDistribution
...
class CompressedFrequencyDistribution:
    ...
    def get_memory_usage(self) -> Dict[str, int]:
        ...
        for k, v in self._id_to_element.items():
            id_to_element_size += sys.getsizeof(k) + sys.getsizeof(v)
        ...
        if self.intern_strings:
            ...
            for k, v in self._string_cache.items():
                string_cache_size += sys.getsizeof(k) + sys.getsizeof(v)
    ...
    @classmethod
    def decompress_from_bytes(cls, compressed_data: bytes, use_compression: bool = True) -> 'CompressedFrequencyDistribution':
        """
...
class QuantizedProbabilityTable:
    ...
    def get_quantization_error_stats(self, original_probs: Dict[Element, float]) -> Dict[str, float]:
        """
        ...
        if not errors:
            return {'mean_absolute_error': 0.0, 'max_absolute_error': 0.0, 
                   'mean_relative_error': 0.0, 'max_relative_error': 0.0}
