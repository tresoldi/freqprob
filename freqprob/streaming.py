...
from typing import Dict, Iterator, Optional, Union, Any, Set, Tuple, List
import threading
import weakref
from .base import ScoringMethod, ScoringMethodConfig, Element, Count, Probability, LogProbability
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                'most_frequent': max(self._counts.items(), key=lambda x: x[1]) if self._counts else None,
...
class StreamingMLE(ScoringMethod, IncrementalScoringMethod):
    ...
    def __init__(
        ...
        self._stream_dist = StreamingFrequencyDistribution(
        ...
        if count > 0:  # Only if we actually added count
            for other_element in self._prob:
                if other_element != element:
                    old_prob = math.exp(self._prob[other_element]) if self.logprob else self._prob[other_element]
                    ...
                    if self.logprob:
                        self._prob[other_element] = math.log(new_prob) if new_prob > 0 else math.log(1e-10)
                    else:
...
class StreamingDataProcessor:
    ...
    def _check_auto_save(self) -> None:
        ...
        if (self.auto_save_interval and 
            self._processed_count % self.auto_save_interval == 0):
