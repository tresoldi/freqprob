...
def _check_probdist_args(freqdist, **kwargs):
    """
    Internal function for validating arguments for the various smoothing functions.
    ...
    if unobs_prob:
        if unobs_prob < 0.0 or unobs_prob > 1.0:
            raise ValueError("Reserved unobserved probability must be in range [0.0, 1.0].")
...
#       methods.
def smooth_dist(freqdist: Dict[str, int], method: str, **kwargs) -> Tuple[Dict, float]:
    """
    ...
    smoothing method to use and passes all the arguments to the appropriate
    function.
...
def uniform_dist(freqdist: Dict[str, int], unobs_prob: float = _UNOBS) -> Tuple[Dict, float]:
    """
...
def random_dist(freqdist: Dict[str, int], unobs_prob: float = _UNOBS, seed: Hashable = None) -> Tuple[Dict, float]:
    """
    ...
    probdist = {sample: math.log((count / fake_sum) * (1.0 - unobs_prob)) for sample, count in fake_count.items()}

    return probdist, math.log(unobs_prob)


def mle_dist(freqdist: Dict[str, float], unobs_prob: float = _UNOBS) -> Tuple[Dict, float]:
    """
    ...
    probdist = {sample: math.log((count / value_sum) * (1.0 - unobs_prob)) for sample, count in freqdist.items()}
...
def lidstone_dist(
    ...
    probdist = {sample: math.log((count + gamma) / (n + b * gamma)) for sample, count in freqdist.items()}
    prob_unk = math.log(gamma / (n + b * gamma))

    return probdist, prob_unk


laplace_dist = partial(lidstone_dist, gamma=1.0)
...
def wittenbell_dist(freqdist: Dict[str, int], bins: Optional[int] = None) -> Tuple[Dict, float]:
    """
    ...
    probdist = {sample: math.log(count / (n + t)) for sample, count in freqdist.items()}
...
def certaintydegree_dist(
    freqdist: Dict[str, int], bins: Optional[int] = None, unobs_prob: float = _UNOBS
) -> Tuple[Dict, float]:
    """
    ...
    from a computation of the degree of certainty that there are no unobserved
    samples.
    ...
    probdist = {sample: math.log((count / n) * prob_space) for sample, count in freqdist.items()}

    # Calculate the unobserved probability from the probability space.
    prob_unk = math.log(-(prob_space - 1.0))
...
def sgt_dist(
    ...
    slope, intercept = linalg.lstsq(np.c_[np.log(z_keys), (1,) * len(z_keys)], np.log(z_values))[0]
