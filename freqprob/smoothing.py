...
from typing import Optional, Dict, List, Tuple, Set, DefaultDict
...
class ModifiedKneserNey(ScoringMethod):
    ...
    def _compute_probabilities(self, freqdist: FrequencyDistribution) -> None:
        ...
        n1, n2, n3, n4 = count_frequencies.get(1, 0), count_frequencies.get(2, 0), count_frequencies.get(3, 0), count_frequencies.get(4, 0)
        ...
        for context in contexts:
            ...
            n1_types = sum(1 for word in context_types[context] 
                          if freqdist.get((context, word), 0) == 1)
            n2_types = sum(1 for word in context_types[context] 
                          if freqdist.get((context, word), 0) == 2)
            n3_plus_types = sum(1 for word in context_types[context] 
                               if freqdist.get((context, word), 0) >= 3)
...
class InterpolatedSmoothing(ScoringMethod):
    ...
    def _compute_probabilities(self, freqdist: FrequencyDistribution) -> None:
        ...
        high_probs = {
            element: count / high_total 
            for element, count in self._high_order_dist.items()
        }
        low_probs = {
            element: count / low_total 
            for element, count in self._low_order_dist.items()
        }
...
class BayesianSmoothing(ScoringMethod):
    ...
    def _compute_probabilities(self, freqdist: FrequencyDistribution) -> None:
        ...
        if self.logprob:
            ...
            self._prob = {
                element: (count + alpha) / denominator
                for element, count in freqdist.items()
            }
